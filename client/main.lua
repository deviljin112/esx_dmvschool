...
function OpenDMVSchoolMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'dmvschool_actions', {
		...
		if data.current.value == 'theory_test' then
			local price = Config.Prices[data.current.type]

			menu.close()

			ESX.TriggerServerCallback('esx_dmvschool:pay', function(cb)
				if cb then
					StartTheoryTest()
				else
...
function StartDriveTest(type)
	...
	ESX.Game.SpawnVehicle(Config.VehicleModels[type], Config.Zones.VehicleSpawnPoint.Pos, Config.Zones.VehicleSpawnPoint.Pos.h, function(vehicle)
		...
		SetVehicleNumberPlateText(vehicle, "LEARNER")
...
function StopDriveTest(success)
	...
	SetEntityCoords(playerPed, Config.Zones.DMVSchool.Pos.x, Config.Zones.DMVSchool.Pos.y, Config.Zones.DMVSchool.Pos.z)
	DoScreenFadeIn(300)
	...
	while true do
		...
		if CurrentTest == 'drive' then
			...
			if IsPedInAnyVehicle(playerPed, false) then
				...
				if IsAboveSpeedLimit and not Triggered then
					Triggered = true
